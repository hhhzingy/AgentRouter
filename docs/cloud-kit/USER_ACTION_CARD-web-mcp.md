...
## 演练 B:Review(参与角色,完整工作环 W1–W7)

- 实栈已预置:`task_989880f1…` 处于 WAITING_INPUT(send_user_input 正向目标)、
  `task_1271f0f7…` QUEUED(口令 WEBDEMO-7QX4 闭环)。Tunnel/token/grant 沿用,不需重配。
- 网页端按 `docs/cloud-kit/CHATGPT-walkthrough.zh.md` 顺序执行 W1–W7:
  inbox 结构化读正文 → 未就绪拦截 → send_user_input(幂等+异参冲突) → 继续认领 →
  产物(≠完成) → submit_result(DELIVERED/PUBLISHED/acceptance 仍 PENDING) → 口令闭环。
- 本机对账:两条 participant result 账目、管理面 `result.accept` 人类验收、撤销演示(GENERATION_STALE)。
