...
| F07 | P0 | FIXED_AUTOMATED_REAL_OBSERVER | Management MCP scope 正负测通过；Cursor observer 曾实连且不是 Role。本 SHA 增加 Slot list/create/leave 与严格 observer 工具过滤；真实 Cursor controller 待跑。 |
| F08 | P1 | FIXED_AUTOMATED_REAL_PARTIAL | AuthContext 已显式分离 authenticated/principal kind/controller eligibility/lease；Remote observer 可读但 hello 明确 `controller_lease=false`。Cursor controller 与真实 Remote 全链未跑。 |
| F09 | P0 | FIXED_AUTOMATED | Participant grant/generation fencing 与 extension principal 授权有负测。 |
| F10 | P0 | FIXED_AUTOMATED | malformed Remote input 局部失败及边界测试通过；extension mutation metadata 现完整透传，不绕过同源授权与幂等规则。 |
| F11 | P0 | FIXED_AUTOMATED_REAL_PENDING | revoke 后端断流与“排队 mutation 不进入 Core”通过；response drop/reconnect 不自动重放 UNKNOWN 写。真实 HTTPS/WSS existing stream 未跑。 |
...
| F18 | P1 | PARTIAL | capability 原值/UNKNOWN 展示合同已冻结；Codex/ZCode Management launcher 自动回归通过，但真实五 Harness Level A/B 声明仍不完整。 |
...
| F25 | P1 | PARTIAL_HOST | credential hash/safeStorage/remote locality 已实现；Remote Device 管理现严格限定 authenticated LOCAL_CLIENT，Remote/Participant/unauthenticated 均拒绝。HTTPS/WSS 真实部署未验证。 |
...
| F29 | P2 | FIXED_AUTOMATED_REAL_PENDING | lease/scope、Slot mutation revision/request_key/operation ledger、observer/controller 工具面均有正负测；真实客户端 controller 细粒度矩阵未跑。 |
...
3. N4 自动 receipt/target 所有权与 N5 自动 Management/Auth/Remote 已收口；下一步补 ZCode 真 receipt/cross-Harness、Cursor controller 和 Tailscale HTTPS/WSS 真实证据。
4. N6 以同一干净候选 SHA 跑五 Harness Level A/B；ZCode、Pi Level B 如仍失败须保留失败分母。
5. N7—N10 补真实网页 Participant、migration/fault、安装包、CI 与 UI 合流；在此之前不宣称 Windows RC。
