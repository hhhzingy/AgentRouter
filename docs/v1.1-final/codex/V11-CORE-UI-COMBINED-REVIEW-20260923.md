...
| 本轮产品与测试 SHA | `8c6f84e34dba5e6a47dfc416de127562073d8ae8`，已推送 GitHub；本文为该源码的文档后继提交，不冒充包内 SHA |
...
| Result 请求修改真实管道 | 网页 Participant 的第二个 Result 经本地 Core pipe `result.requestChanges`，原 Result 仍 `PUBLISHED`，反馈、后续 Task 与数据库关系可复核；另有 Core 原子/幂等/权限/回滚定向测试。此项尚不是 Electron UI 点击的端到端验收。 |
...
3. GAP-004 Core 原子 `result.requestChanges` 与 UI 消费已合流；尚缺最终包的 Electron UI→真实 Core→Result/Task 数据库闭环与未知回执状态复核。
4. `8c6f84e` 同 SHA 的 Codex、ZCode、Pi、Kimi、DSH 真实任务与冷恢复、Artifact 工作流未全套重跑。旧 SHA 的通过记录保留为历史证据，不自动迁移成联合 SHA PASS。
5. 最终包的 HTTPS/WSS Remote、手机控制/观察者、WorkSession 真实迁移与屏幕阅读器/DPI 验收未完成；当前包还不是安装包或发布候选。
6. 合流 SHA 的 C1/W11 已实际 completed/success；还需完成真实 Harness/Remote/Mobile/页面证据的同 SHA 绑定，不能将此 CI 绿灯扩大为 Windows RC。
