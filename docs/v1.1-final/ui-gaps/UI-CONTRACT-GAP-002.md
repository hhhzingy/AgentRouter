...
2026-09-23 再后继：新增 v20 additive migration `result_evidence_attestations` 和控制者专用 `result.evidence.record`。每个已发布 Result 只允许一份不可变声明；源码修订限定完整 40/64 位十六进制 Git SHA，测试记录限定 `PASS/FAIL/SKIP/UNKNOWN` 且引用的证据 Artifact 必须属于该 Result，限制清单有界。写入要求 Controller Lease、预期修订与幂等操作 ID；审计、回滚、重放及 v19→v20 升级前备份均有测试。读取投影明确返回 `CONTROLLER_ATTESTED`，UI 说明这三类字段是 Controller 声明，并非 Core 自动核验 Git 对象或测试过程。无声明时仍显示 `NOT_RECORDED`。本项完成的是持久契约与隔离测试，不是已有真实 Result 被自动补证；真实数据写入、Electron 页面与最终同 SHA 包/CI 仍需独立验收，GAP 保持 PARTIAL。
