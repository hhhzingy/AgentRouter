...
public static class TokenCanary {
 ...
 static void Primary(IntPtr token,string workspace,string secret,string desktop=null,bool nativeControl=false,bool nodeControl=false) {
  ...
  string nodeScript="const fs=require('fs'),cp=require('child_process'),p=process.argv[1],s=process.argv[2],d=process.argv[3];let nulReadErrno=0,nulWriteErrno=0;for(const mode of ['r','w']){try{fs.closeSync(fs.openSync(String.fromCharCode(92).repeat(2)+'.'+String.fromCharCode(92)+'NUL',mode))}catch(e){if(mode==='r')nulReadErrno=Number.isInteger(e.errno)?e.errno:-999;else nulWriteErrno=Number.isInteger(e.errno)?e.errno:-999}}let phase=1,aclSpawnError=false,aclExit=-999,aclErrno=0,canaryStillDenied=false;try{fs.writeFileSync(p+'/node-'+d+'.txt','dummy');if(fs.readFileSync(p+'/node-'+d+'.txt','utf8')!=='dummy')throw 0;phase=2;for(const action of [()=>fs.readFileSync(s+'/canary.txt'),()=>fs.appendFileSync(s+'/canary.txt','dummy')]){let denied=false;try{action()}catch(e){denied=e.code==='EACCES'||e.code==='EPERM'}if(!denied)throw 0}phase=3;const dummyFd=fs.openSync(p+'/stdio-'+d+'.txt','w+');const acl=cp.spawnSync(process.env.SystemRoot+'/System32/icacls.exe',[s,'/grant','*S-1-5-32-545:R'],{stdio:[dummyFd,dummyFd,dummyFd],timeout:3000,env:{SystemRoot:process.env.SystemRoot}});aclSpawnError=Boolean(acl.error);aclErrno=Number.isInteger(acl.error?.errno)?acl.error.errno:0;aclExit=Number.isInteger(acl.status)?acl.status:-999;try{fs.readFileSync(s+'/canary.txt')}catch(e){canaryStillDenied=e.code==='EACCES'||e.code==='EPERM'}if(acl.status===0||acl.error||!canaryStillDenied)throw 0;phase=4;if(d==='0'){const child=cp.spawnSync(process.execPath,[...process.execArgv,p,s,'1'],{stdio:[dummyFd,dummyFd,dummyFd],timeout:5000,env:{SystemRoot:process.env.SystemRoot}});if(child.status!==0||child.error)throw 0}phase=5;fs.writeFileSync(p+'/node-'+d+'-pass.json',JSON.stringify({nulReadErrno,nulWriteErrno}));process.exit(0)}catch{try{fs.writeFileSync(p+'/node-'+d+'-failure.json',JSON.stringify({phase,aclSpawnError,aclExit,aclErrno,canaryStillDenied}))}catch{}process.exit(1)}";
  ...
  try{if(!CreateProcessAsUser(token,exe,new System.Text.StringBuilder(nodeControl?"\""+exe+"\" -e \""+nodeScript+"\" \""+workspace+"\" \""+secret+"\" 0":nativeControl?"\""+exe+"\" /d /c exit 0":"\""+exe+"\" --child \""+workspace+"\" \""+secret+"\" 0"),childAttributes,childAttributes,false,0x08000400,environment,workspace,ref si,out pi))throw new Win32Exception(Marshal.GetLastWin32Error());
   try{if(WaitForSingleObject(pi.process,15000)!=0){TerminateProcess(pi.process,124);throw new TimeoutException();}uint code;if(!GetExitCodeProcess(pi.process,out code))throw new Win32Exception(Marshal.GetLastWin32Error());Console.WriteLine("{\"primaryChildExitCode\":"+code+"}");if(nodeControl){string failure=Path.Combine(workspace,"node-0-failure.json");if(File.Exists(failure)){var diagnostic=File.ReadAllText(failure);if(System.Text.RegularExpressions.Regex.IsMatch(diagnostic,@"^\{""phase"":\d,""aclSpawnError"":(true|false),""aclExit"":-?\d+,""aclErrno"":-?\d+,""canaryStillDenied"":(true|false)\}$"))Console.WriteLine(diagnostic);}}Require(code==0);if(nodeControl){Require(File.Exists(Path.Combine(workspace,"node-0-pass.json"))&&File.Exists(Path.Combine(workspace,"node-1-pass.json")));var nr=File.ReadAllText(Path.Combine(workspace,"node-0-pass.json"));if(System.Text.RegularExpressions.Regex.IsMatch(nr,@"^\{""nulReadErrno"":-?\d+,""nulWriteErrno"":-?\d+\}$"))Console.WriteLine(nr);}else if(!nativeControl)Require(File.Exists(Path.Combine(workspace,"child-0-pass.json"))&&File.Exists(Path.Combine(workspace,"child-1-pass.json")));}finally{CloseHandle(pi.thread);CloseHandle(pi.process);}
