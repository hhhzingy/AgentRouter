...
try{
 ...
 expect(await harnessSelect.inputValue()).toBe(slotRole.harness);
 await expect(page.getByText('目标 Harness Preflight：'+slotRole.harness,{exact:true})).toBeVisible();
 await expect(page.getByRole('button',{name:'下一步'})).toBeDisabled();
 await expect(page.getByText('当前 Harness 未声明可创建 WorkSession。',{exact:true})).toBeVisible();
 await page.getByRole('button',{name:'取消',exact:true}).click();
 pass('J2_WORKSESSION_PREFLIGHT_SAFE_GATE');
