...
try{
 ...
 await page.getByRole('button',{name:'新建 WorkSession'}).click();
 const harnessSelect=page.locator('.wizard-panel select');
 const alternative=await harnessSelect.locator('option:not([disabled])').evaluateAll((options:Element[],current:string)=>options.map((x:Element)=>(x as HTMLOptionElement).value).find((x:string)=>x!==current)??'',String(slotRole.harness));
 expect(alternative).toBeTruthy();
 await harnessSelect.selectOption(alternative);
 await expect(page.getByText('目标 Harness Preflight：'+alternative,{exact:true})).toBeVisible();
 await page.getByRole('button',{name:'下一步'}).click();
 await page.getByText('Transfer from current WorkSession',{exact:true}).click();
 await expect(page.getByText(/容量：未测量，不能确认 Fits 或需要压缩/)).toBeVisible();
 await page.getByRole('button',{name:'取消',exact:true}).click();
 pass('J2_TARGET_HARNESS_PREFLIGHT_UI');
 ...
 await page.evaluate((id:string)=>{location.hash='#/project/'+id;},project.id);await expect(page.locator('aside[aria-label="项目导航"] nav a')).toHaveCount(5);
 await page.locator(`a[href="#/project/${project.id}/timeline"]`).click();await page.getByLabel('动态角色',{exact:true}).selectOption(a.id);await expect(page.getByRole('button',{name:/显示 \d+ 条工具事件/})).toBeVisible();await page.getByRole('button',{name:/显示 \d+ 条工具事件/}).click();await expect(page.getByText('A_独立资料 历史 0',{exact:true})).toBeVisible();expect(await page.getByText('B_独立资料 历史 0',{exact:true}).count()).toBe(0);await shot('project-activity');await page.locator(`a[href="#/project/${project.id}/inbox"]`).click();await page.getByRole('button',{name:'文件与报告'}).click();await expect(page.locator('[data-tab="artifacts"]')).toBeVisible();await page.locator(`a[href="#/project/${project.id}/settings"]`).click();await page.getByRole('button',{name:'运行环境'}).click();await expect(page.locator('[data-tab="models"]')).toBeVisible();await shot('runtime-settings');await page.goBack();await expect(page.locator('[data-tab="settings"]')).toBeVisible();pass('J2_FOUR_ENTRIES_FILTERS_BACK');
 ...
 await page.evaluate((id:string)=>{location.hash='#/project/'+id;},project.id);await expect(page.locator(`a[href="#/project/${project.id}"]`)).toBeVisible();
 ...
 await page.reload();await page.getByTestId('core-identity').getByText('Connected').waitFor();await page.getByRole('button',{name:'申请控制'}).click();
