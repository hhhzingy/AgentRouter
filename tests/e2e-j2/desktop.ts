...
const sourceDirty=execFileSync('git',['-c','safe.directory='+process.cwd().replaceAll('\\','/'),'diff','HEAD','--name-only','--','apps','packages','tests','tools'],{encoding:'utf8'}).trim().length>0;
...
try{
 ...
 const contrasts=await page.evaluate(()=>{const css=getComputedStyle(document.documentElement),lum=(hex:string)=>{const n=hex.trim().replace('#','');const c=[0,2,4].map(i=>parseInt(n.slice(i,i+2),16)/255).map(v=>v<=.04045?v/12.92:((v+.055)/1.055)**2.4);return c[0]*.2126+c[1]*.7152+c[2]*.0722;};return ['text','text-2','text-3','ok','warning','danger','queue','active'].map(name=>{const fg=lum(css.getPropertyValue('--'+name)),bg=lum(css.getPropertyValue(name.startsWith('text')?'--bg':'--'+name+'-soft'));return {name,ratio:(Math.max(fg,bg)+.05)/(Math.min(fg,bg)+.05)};});});for(const color of contrasts)expect(color.ratio,JSON.stringify(color)).toBeGreaterThanOrEqual(4.5);writeFileSync('evidence/J2/contrast.json',JSON.stringify(contrasts,null,2));pass('J2_TEXT_CONTRAST');
 ...
 await core?.stop();writeFileSync('evidence/J2/desktop.json',JSON.stringify({sourceSha,sourceDirty,mode:'REAL_ELECTRON_LOCAL_CORE',realHarnessSupport:0,nativeDialogs:'CONTROLLED_RETURN_MANUAL_NOT_RUN',checks,screenshots},null,2)+'\n');
}
