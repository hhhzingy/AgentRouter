...
try{
 ...
 for(const factor of [1.25,1.5,2]){await app.evaluate(({BrowserWindow}:any,z:number)=>{const w=BrowserWindow.getAllWindows()[0];w.setContentSize(1280,720);w.webContents.setZoomFactor(z);},factor);await expect(page.getByRole('button',{name:'添加角色',exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+2&&document.body.scrollWidth<=innerWidth+2)).toBe(true);const action=await page.getByRole('button',{name:'添加角色',exact:true}).boundingBox();expect(action.x+action.width).toBeLessThanOrEqual(await page.evaluate(()=>innerWidth));await shot('workbench-zoom');}pass('J2_ZOOM_125_150_200');
