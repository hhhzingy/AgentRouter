...
it('旧8个项目路由仍有明确功能落点，项目页不重复渲染顶级页签',()=>{const s=makeStore({}),id=s.snapshot.projects[0].id;for(const tab of ['overview','spaces','timeline','inbox','issues','artifacts','models','settings']){const html=page(s,<ProjectPage projectId={id} tab={tab}/>);expect(html).toContain(`data-tab="${tab}"`);expect(html).not.toContain('role="tablist"');}});
