...
export function RolePlanPage({projectId}:{projectId:string}){
 ...
 <section className="card"><h2>导入方案（JSON）</h2><p>仅支持 JSON，最多 128 KiB；外部项目与工作区 ID 必须映射确认。</p><input aria-label="导入 Role Plan JSON" type="file" accept=".json,application/json" onChange={async e=>{const f=e.target.files?.[0];if(!f)return;if(f.size>131072){setError('方案超过 128 KiB。');return;}const token=++epoch.current;try{const text=await f.text();if(alive.current&&token===epoch.current)await importText(text);}catch(e){if(alive.current&&token===epoch.current){setAction('failed');setError(errorMessage(e));}}}}/><textarea aria-label="粘贴协作方案 JSON" value={paste} onChange={e=>setPaste(e.target.value)} maxLength={131072}/><button disabled={!paste.trim()} onClick={()=>void importText(paste)}>校验粘贴方案</button></section>
