...
export function HistoryPanel({scope,roleId,businessFirst=false}:{scope:Scope;roleId?:string;businessFirst?:boolean}) {
 const s=useStore();
 const key=JSON.stringify([s.hello.serverInstanceId,scope,roleId]);
 return <ScopedHistory key={key} scope={scope} roleId={roleId} businessFirst={businessFirst}/>;
}
function ScopedHistory({scope,roleId,businessFirst}:{scope:Scope;roleId?:string;businessFirst:boolean}) {
 ...
 const technicalCount=items.filter(item=>item.kind==='TOOL_CALL'||item.kind==='TOOL_RESULT').length;
 const visibleItems=businessFirst&&!showTechnical?items.filter(item=>item.kind!=='TOOL_CALL'&&item.kind!=='TOOL_RESULT'):items;
 return <section className="history-panel" aria-label="历史记录">
  ...
  {businessFirst&&technicalCount>0&&<button className="btn btn-secondary" aria-pressed={showTechnical} onClick={()=>setShowTechnical(value=>!value)}>{showTechnical?'隐藏工具事件':`显示 ${technicalCount} 条工具事件`}</button>}
  ...
  {visibleItems.length>0 && <ConversationView items={visibleItems} now={s.now()}/>}
