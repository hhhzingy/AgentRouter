...
function ResultDetail({result}:{result:ReturnType<typeof useStore>['snapshot']['results'][number]|undefined}){
 const s=useStore();
 const [evidence,setEvidence]=useState<{evidence_layer:string;execution_layer:'REAL_NATIVE'|'FIXTURE'|'UNKNOWN';source_revision:string|null;run_id:string|null;harness:string|null;model_id:string|null;artifacts:Array<{id:string;sha256:string|null;state:string}>;tests:Array<{name:string;outcome:'PASS'|'FAIL'|'SKIP'|'UNKNOWN';evidence_artifact_id?:string}>;test_records_status:string;known_limitations:string[]|null;attestation:{source:'CONTROLLER_ATTESTED';recorded_at_ms:number}|null}|null>(null);
 ...
 return <Card className="result-detail"><span className="eyebrow">RESULT DETAIL</span><h2>{result.summary}</h2>
  ...
  {evidence?<div className="result-evidence"><KeyValue k="证据来源" v="Run / Artifact 为 Core 持久事实；源码、测试和限制为 Controller 声明"/><KeyValue k="执行层" v={evidence.execution_layer==='REAL_NATIVE'?'真实 Native Run（不等于测试通过）':evidence.execution_layer==='FIXTURE'?'隔离 Fixture Run':'未知，不能推断为真实执行'}/><KeyValue k="源码修订" v={evidence.source_revision?`${evidence.source_revision}（Controller 声明，Core 未核验 Git 对象）`:'未记录，不能推断'}/><KeyValue k="结构化测试记录" v={evidence.test_records_status==='NOT_RECORDED'?'未记录，不能视为通过':`Controller 声明 ${evidence.tests.length} 条，非 Core 自动测试证明`}/>{evidence.tests.map((t,i)=><p key={`${t.name}-${i}`}>{t.name} · 声明 {t.outcome}{t.evidence_artifact_id?` · 关联 ${t.evidence_artifact_id}`:''}</p>)}<KeyValue k="已知限制" v={evidence.known_limitations===null?'未记录，不代表无限制':evidence.known_limitations.length?evidence.known_limitations.join('；'):'Controller 声明无已知限制'}/><KeyValue k="已核验产物" v={evidence.artifacts.length}/>{evidence.artifacts.map(a=><p key={a.id}>{a.id} · {a.state} · SHA-256 {a.sha256??'未提供'}</p>)}</div>:<p className="muted">{evidenceError?`Evidence 暂不可用：${evidenceError}`:'正在读取 Core Evidence…'}</p>}
