...
export function errorMessage(error: unknown) {
 ...
 const messages: Record<string,string> = {REQUEST_TIMEOUT:'提交结果尚未确认，请核对后按原操作重试。',CONNECTION_LOST:'连接已断开，提交结果可能需要核对。',CONTROL_LEASE_REQUIRED:'请先申请控制权。',CONTROL_LEASE_EXPIRED:'控制权已过期，请重新申请。',CAPABILITY_UNAVAILABLE:'当前 Core 尚不支持此操作。',SCOPE_DENIED:'该对象不在当前授权范围。',REVISION_CONFLICT:'数据已更新，请刷新并重新检查。',INVALID_PARAMS:'内容或引用无效，请检查输入。'};
