...
export type PendingMethod = Method | 'roleSession.create' | 'roleSession.switch' | 'result.requestChanges' | 'participant.slot.create' | 'participant.leave';
...
export class PendingStore {
 ...
 prepare(method:PendingMethod,params:unknown,revision:number,scope:Scope,preflightHash?:string){
  ...
  if(method.startsWith('roleSession.')||method==='participant.slot.create'||method==='participant.leave')record.requestKey=record.operationId;
