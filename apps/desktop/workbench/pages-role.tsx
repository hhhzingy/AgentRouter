...
function SessionWorkflow({ role,onWebParticipant }: { role:RoleVM;onWebParticipant:()=>void }) {
  ...
  return (
    <Card className="session-card">
      ...
      {wizardOpen&&<CreateWorkSessionWizard role={role} busy={busy} onClose={()=>setWizardOpen(false)} onWebParticipant={()=>{setWizardOpen(false);onWebParticipant();}} onCreate={async params=>{await act('roleSession.create',params);setWizardOpen(false);}}/>}
    </Card>
  );
}

function CreateWorkSessionWizard({role,busy,onClose,onWebParticipant,onCreate}:{role:RoleVM;busy:boolean;onClose:()=>void;onWebParticipant:()=>void;onCreate:(params:Record<string,unknown>)=>Promise<void>}){
 const s=useStore();
 ...
 const [targetPreflight,setTargetPreflight]=useState<RoleSessionPreflight|null>(null),[targetPreflightError,setTargetPreflightError]=useState('');
 const [harnesses,setHarnesses]=useState<Array<{id:string;status:string;create_session:boolean}>>(()=>Object.entries(s.capabilities.harnesses).map(([id,cap])=>({id,status:cap.status,create_session:cap.create_session})));
 const [harnessesError,setHarnessesError]=useState('');
 React.useEffect(()=>{let active=true;void s.callExtension('roleSession.harnesses',{role_id:role.id}).then(value=>{if(!active)return;const list=(value as {harnesses?:Array<{id:string;status:string;create_session:boolean}>}).harnesses;if(Array.isArray(list)&&list.length)setHarnesses(list);setHarnessesError('');},e=>{if(active)setHarnessesError(errorMessage(e));});return()=>{active=false;};},[role.id,s]);
 React.useEffect(()=>{let active=true;setTargetPreflight(null);setTargetPreflightError('');void s.callExtension('roleSession.preflight',{role_id:role.id,target_harness:harness}).then(value=>{if(active)setTargetPreflight(value as RoleSessionPreflight);},e=>{if(active)setTargetPreflightError(errorMessage(e));});return()=>{active=false;};},[role.id,harness,s]);
 const selectedHarnessAvailable=Boolean(harnesses.find(item=>item.id===harness)?.create_session);
 const preflightReady=targetPreflight?.target_harness===harness&&typeof targetPreflight.preflight_hash==='string';
 const blocked=contextMode==='inherit'&&targetPreflight?.migration_fidelity==='BLOCKED';
 const submit=async()=>{setError('');try{if(!preflightReady)throw Error('PREFLIGHT_REQUIRED');const latest=await s.callExtension('roleSession.preflight',{role_id:role.id,target_harness:harness}) as RoleSessionPreflight;if(latest.preflight_hash!==targetPreflight.preflight_hash){setTargetPreflight(latest);throw Error('PREFLIGHT_CHANGED_REVIEW_AGAIN');}await onCreate({role_id:role.id,name:name.trim()||`工作会话 · ${role.name}`,target_harness:harness,context_mode:contextMode});}catch(e){setUncertain(failureState(e)==='uncertain');setError(errorMessage(e));}};
 return <Dialog title="新建 WorkSession" onClose={onClose} footer={<><Button onClick={onClose}>{uncertain?'关闭并保留待核对操作':'取消'}</Button>{step>0&&!uncertain&&<Button onClick={()=>setStep(step-1)}>上一步</Button>}{sessionType==='web'?<Button variant="primary" onClick={onWebParticipant}>继续创建 Web Participant Slot</Button>:step<2?<Button variant="primary" disabled={blocked||uncertain||!selectedHarnessAvailable||!preflightReady} onClick={()=>setStep(step+1)}>下一步</Button>:!uncertain&&<Button variant="primary" disabled={busy||blocked||!selectedHarnessAvailable||!preflightReady} onClick={()=>void submit()}>{busy?'正在创建…':'创建 WorkSession'}</Button>}</>}>
  ...
  {step===0&&<div className="wizard-panel"><h3>谁来承担这段上下文</h3><KeyValue k="Role" v={role.name}/><label className={`context-option ${sessionType==='managed'?'selected':''}`}><input type="radio" name="session-type" checked={sessionType==='managed'} onChange={()=>setSessionType('managed')}/><span><b>Managed Harness</b><small>由 Core 创建和管理原生会话。</small></span></label><label className={`context-option ${sessionType==='web'?'selected':''}`}><input type="radio" name="session-type" checked={sessionType==='web'} onChange={()=>setSessionType('web')}/><span><b>Web Participant</b><small>先创建 Slot，再用 Join Instruction 邀请参与者；绑定状态由 Core 确认。</small></span></label>{sessionType==='managed'?<><details><summary>可选：命名 WorkSession</summary><label className="field"><span>名称</span><input type="text" maxLength={80} value={name} onChange={e=>setName(e.target.value)} placeholder={`工作会话 · ${role.name}`}/></label></details><label className="field"><span>Managed Harness</span><select value={harness} onChange={e=>{setHarness(e.target.value as RoleVM['harness']);setTargetPreflight(null);}}>{[...harnesses,...(!harnesses.some(item=>item.id===harness)?[{id:harness,status:'UNKNOWN',create_session:false}]:[])].map(item=><option key={item.id} value={item.id} disabled={!item.create_session}>{item.id} · {item.status}{item.create_session?'':' · 不支持创建'}</option>)}</select></label>{!selectedHarnessAvailable&&<p className="hint tone-warning">当前 Harness 未声明可创建 WorkSession。</p>}{harnessesError&&<p className="hint tone-warning">动态 Harness 能力暂不可用，仅显示冻结合同内的安全子集：{harnessesError}</p>}{targetPreflightError?<p role="alert">目标 Harness Preflight 不可用：{targetPreflightError}</p>:<p className="hint">目标 Harness Preflight：{preflightReady?targetPreflight.target_harness:'正在向 Core 查询…'}</p>}<p className="hint">Harness、workspace 与 native session 在 WorkSession 建立后不能静默更换。</p></>:<p className="hint">下一步只会准备一个 Participant 槽位。参与者加入并由 Core 确认前，不会显示为活跃 WorkSession。</p>}</div>}
  {step===1&&<div className="wizard-panel"><h3>选择 Context 策略</h3><label className={`context-option ${contextMode==='blank'?'selected':''}`}><input type="radio" name="context" checked={contextMode==='blank'} onChange={()=>setContextMode('blank')}/><span><b>Start blank</b><small>创建全新上下文，不复制当前 WorkSession。</small></span></label><label className={`context-option ${contextMode==='inherit'?'selected':''}`}><input type="radio" name="context" checked={contextMode==='inherit'} onChange={()=>setContextMode('inherit')}/><span><b>Transfer from current WorkSession</b><small>Core 决定可迁移内容与保真度；UI 不估算百分比。</small></span></label>{contextMode==='inherit'&&<div className={`hint ${blocked?'tone-warning':''}`}>Capability: {targetPreflight?.migration_fidelity??'UNKNOWN'} · {preflightReasonLabel(targetPreflight?.reason_code)}{blocked&&' 当前迁移被 Core 阻止，请改用 Start blank。'}<p>容量：{targetPreflight?.capacity_assessment?.status==='UNKNOWN'?'未测量，不能确认 Fits 或需要压缩':targetPreflight?.capacity_assessment?.status??'Core 未提供'} · 压缩：{targetPreflight?.compression_policy==='CORE_DECIDES'?'由 Core 决定':targetPreflight?.compression_policy??'未上报'}</p></div>}</div>}
  {step===2&&<div className="wizard-panel"><h3>确认创建</h3><KeyValue k="Role" v={role.name}/><KeyValue k="Session Type" v="Managed Harness"/><KeyValue k="Harness" v={harness}/><KeyValue k="Context" v={contextMode==='blank'?'Start blank':'Transfer from current WorkSession'}/><KeyValue k="创建前容量评估" v={targetPreflight?.capacity_assessment?.status==='UNKNOWN'?'未知，Core 尚未测量':targetPreflight?.capacity_assessment?.status??'Core 未提供'}/><KeyValue k="压缩决策" v={targetPreflight?.compression_policy==='CORE_DECIDES'?'由 Core 决定':targetPreflight?.compression_policy??'未上报'}/><KeyValue k="当前任务影响" v="只有 Core 成功激活后才切换；失败时当前 WorkSession 保持安全。"/><p className="hint tone-warning">旧 WorkSession 在成功切换后进入 Historical · Read-only，不能恢复。</p></div>}
