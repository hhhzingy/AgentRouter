...
const source = readFileSync('contracts/client-api.c1r1p1.schema.json', 'utf8').replaceAll(
  '\r\n',
  '\n',
);
...
const names = {
  key: 'PlanKey',
  target: 'PlanTarget',
  model_selection: 'PlanModelSelection',
...
function remap(value) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([k, v]) => [
      k,
      k === '$ref' && typeof v === 'string' ? '#/$defs/' + names[v.split('/').at(-1)] : remap(v),
    ]),
  );
}
...
for (const [key, name] of Object.entries(names))
  if (!same(remap(product.$defs[key]), schema.$defs[name]))
    throw Error('ROLE_PLAN_EMBEDDED_DRIFT:' + name);
const { $schema: dialect, $id: id, title, $defs, ...root } = product;
if (!same(remap(root), schema.$defs.RolePlanInput))
  throw Error('ROLE_PLAN_EMBEDDED_DRIFT:RolePlanInput');
...
function type(s) {
  ...
  if (s.type === 'object') {
    if (s.additionalProperties !== false) {
      if (s.maxProperties === 64) return 'Record<string, unknown>';
      throw Error('Unbounded object');
    }
...
let output = `// 自动生成；唯一来源 contracts/client-api.c1r1p1.schema.json。禁止手改。\n// SHA256 ${hash}\n`;
...
const doc =
  ...
  '\n';
for (const [p, text] of [
  ['packages/client-contract/c1r1p1/generated.ts', output],
  ['docs/api/c1r1p1/methods.md', doc],
]) {
  if (process.argv.includes('--check')) {
    if (readFileSync(p, 'utf8') !== text) throw Error('CONTRACT_DRIFT:' + p);
  } else writeFileSync(p, text);
}
console.log('C1R1P1 generation ' + (process.argv.includes('--check') ? 'verified' : 'written'));
