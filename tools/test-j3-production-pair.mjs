...
try {
  ...
  plan.roles = profiles.map((profile, i) => ({ ...structuredClone(template), role_key: `pair_role_${i}`, display_name: `Pair ${profile.harness}`, group_key: plan.groups[0].group_key, workspace_ref: workspace.id, mission: '只完成被明确请求的最小 Route 交接；不使用文件系统或网络工具。', runtime: { harness: profile.harness, provider_profile_id: profile.providerId, model_id: profile.modelId, reasoning_effort: profile.effort, selection_source: 'runtime' }, requested_permissions: { workspace_access: 'read_only', allowed_paths: [], tool_profiles: ['route_context','route_send','route_finish','route_wait'], network_profile: 'none' } }));
