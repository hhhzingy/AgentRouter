...
const pi = owner.profiles?.find(p => p.harness === 'pi');
const codex = owner.profiles?.find(p => p.harness === 'codex');
if (!pi || !codex || pi.modelId !== 'deepseek-v4-flash' || pi.effort !== 'off' || codex.modelId !== 'gpt-5.6-luna' || codex.effort !== 'low') throw Error('APPROVED_PAIR_PROFILES_REQUIRED');
...
const runtime = { ...owner, managedRoot: resolve('.local'), workspaceRoot: path('workspace'), profiles: [{ ...pi, sessionHome: path('managed/pi') }, { ...codex, sessionHome: resolve('.local/j3-codex/dut-fj/home') }] };
...
try {
  ...
  await poll(() => {
    ...
    return report.nativeBootstrap.length === 2 && report.nativeBootstrap.every(row => row.state === 'DELIVERED' && row.source === 'NATIVE');
  });
  ...
  if (pair.length !== 2 || pair[0].space_id !== pair[1].space_id) throw Error('PAIR_NOT_SAME_GROUP');
  ...
  for (const sourceHarness of ['pi','codex']) {
    const source = pair.find(row => row.harness === sourceHarness), target = pair.find(row => row.harness !== sourceHarness);
    ...
    const task = await call('router_task_dispatch', { request_key: `pair-${sourceHarness}`, expected_revision: (await call('router_status')).snapshot.revision, scope: { project_id: project.id, space_id: source.space_id }, params: { request } });
