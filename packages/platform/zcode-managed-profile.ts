...
export interface ZcodeModelProviderConfig {
  ...
  provider: { id: string; kind: 'anthropic' | 'openai' | 'openai-compatible'; baseURL?: string; name?: string };
}
  ...
  if (model) {
    if (
      ...
      (model.provider.kind === 'openai-compatible' &&
        (!model.provider.baseURL || !/^https:\/\/[A-Za-z0-9._~-]+(:\d{1,5})?(\/[!$&'*+,;=:@A-Za-z0-9._~-]*)*\/v\d+$/.test(model.provider.baseURL)))
    )
  ...
  for (const [key, want] of Object.entries(managed)) {
    ...
    if (key === 'provider') {
      ...
      for (const [pid, pwant] of Object.entries(want as Record<string, unknown>)) {
        ...
        if (typeof pgot !== 'object' || pgot === null) throw Error('ZCODE_EXISTING_CONFIG_CONFLICT');
        const { apiKey: _omit, ...rest } = pgot as Record<string, unknown>;
        const wOpt = (pwant as Record<string, unknown>).options as Record<string, unknown> | undefined;
