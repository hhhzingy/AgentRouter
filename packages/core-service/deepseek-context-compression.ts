...
import type { ContextCompressionBackend, ContextCompressionBackendInput } from './context-migration.ts';
...
export class DeepSeekContextCompressionBackend implements ContextCompressionBackend {
  ...
  async compress(input: ContextCompressionBackendInput) {
    ...
    try {
      ...
      const messages = [{ role: 'system', content: instructions },
        { role: 'user', content: stablePortableJson({ sources, entries: input.entries }) }];
      const maxOutput = Math.min(this.config.maxOutputTokens, input.budgetTokens);
      const request = { model: this.model, messages, max_tokens: maxOutput, stream: false,
