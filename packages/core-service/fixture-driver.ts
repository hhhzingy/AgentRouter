...
/** 仅隔离 fixture 数据目录启用；无 Client API 控制口，不认证任何真实 Harness。 */
export class FixtureDriver {
  ...
  private tick() {
    const a = this.app;
    for (const intent of a.all("select * from cancel_intents where state='PENDING'")) {
      const child = this.children.get(intent.run_id);
      if (child) {
        a.db.prepare("update cancel_intents set state='SENT' where run_id=?").run(intent.run_id);
        child.stdin.write(JSON.stringify({ cancel: true, epoch: intent.epoch }) + '\n');
      }
    }
      ...
      (code) => {
        a.db
          .transaction(() => {
            ...
            const success = code === 0 && delivered && terminal && current?.epoch === b.epoch;
            a.db
              .prepare('update initialization_attempts set state=?,completed_at_ms=? where id=?')
              .run(success ? 'DELIVERED' : 'FAILED', a.clock(), attempt);
            a.db
              .prepare(
              ...
              .run(
                ...
                success ? null : 'BOOTSTRAP_EXECUTION_FAILED',
                a.clock(),
                d.id,
              );
