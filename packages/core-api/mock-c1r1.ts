...
export class MockC1R1Server {
  ...
  private caps(c: Connection) {
    const methods = [...reads, ...writes, ...(c.revision === 'C1R1' ? added : [])];
    return {
      methods,
    ...
    try {
      ...
      if (r.method === 'system.initialize') {
        ...
        result = {
          ...
          schemaVersion: c.revision === 'C1R1' ? 2 : 1,
      ...
      if (c.revision === 'C1') result = legacyProjection(r.method, result);
  ...
  private mutate(cid: string, c: Connection, r: any) {
    ...
    if (r.method.startsWith('control.')) {
      ...
      result = this.write(draft, r.method, r.params, r.scope, c.principal);
      validateResponse(r.method, result);
      if (this.failNext) {
        this.failNext = false;
  ...
  private write(product: MockProduct, m: Method, p: any, scope: any, actor: string): any {
